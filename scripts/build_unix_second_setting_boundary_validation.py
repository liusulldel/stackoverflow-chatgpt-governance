...
UNIX_DIR = RAW / "unix_extract"
POSTS_XML = UNIX_DIR / "Posts.xml"
VOTES_XML = UNIX_DIR / "Votes.xml"

OUT_QUESTION = PROCESSED / "unix_second_setting_question_level.parquet"
OUT_PANEL = PROCESSED / "unix_second_setting_tag_month_panel.csv"
OUT_RESULTS = PROCESSED / "unix_second_setting_results.csv"
OUT_SUMMARY = PROCESSED / "unix_second_setting_summary.json"
OUT_MEMO = PAPER / "who_still_answers_unix_second_setting_readout_2026-04-04.md"
    ...
    for _, elem in context:
        ...
        if post_type == "1":
            ...
            if accepted_answer_id:
                focal_accepted_answer_ids.add(accepted_answer_id)
        elif post_type == "2":
...
def build_exposure_scores(df: pd.DataFrame) -> pd.DataFrame:
    ...
    unix_df = df.copy()
    unix_df["question_created_at"] = pd.to_datetime(unix_df["question_created_at"], utc=True)
    unix_df["month_id"] = unix_df["question_created_at"].dt.strftime("%Y-%m")
    unix_df["post_chatgpt"] = (unix_df["question_created_at"] >= SHOCK_DATE).astype(int)
    unix_df["combined_text"] = (
        unix_df["title"].fillna("").astype(str).str.strip()
        + " [TAGS] "
        + unix_df["question_tags"].fillna("").astype(str).str.replace(";", " ", regex=False)
    )
    ...
    x_site, _, _ = exposure_model.build_design(unix_df[score_cols], vectorizer=vectorizer, encoder=encoder)
    unix_df["predicted_exposure_score"] = model.predict(x_site)
    unix_df["predicted_exposure_within_tag_z"] = unix_df.groupby("primary_tag")[
...
def build_question_level() -> pd.DataFrame:
    ...
    if questions.empty:
        raise RuntimeError("No unix second-setting questions extracted.")
    accepted_answer_ids = set(
        questions["accepted_answer_id"].dropna().astype(int).tolist()
    )
    ...
    first_positive = first_answers.rename(columns={"first_answer_at": "first_positive_at"})
    ...
    df = questions.merge(first_answers, on="question_id", how="left").merge(
        first_positive, on="question_id", how="left"
    ).merge(accept_dates, on="accepted_answer_id", how="left")
    cutoff = max(
    ...
    for name, source_col, days in [
        ...
        df[f"{name}_eligible"] = (
            df["question_created_at"] <= (cutoff - pd.Timedelta(days=days))
        ).astype(int)
    ...
    pre_means["high_tag"] = (
        pre_means["pre_exposure_mean"].rank(method="first", ascending=False)
        <= (len(pre_means) / 2)
    ).astype(int)
...
def build_panel(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for (tag, month_id), g in sample.groupby(["primary_tag", "month_id"], sort=True):
        ...
        fa7, fa7_n = rate("first_answer_7d", "first_answer_7d_eligible")
        ...
        rows.append(
            {
                ...
                "first_answer_7d_rate": fa7,
                "first_answer_7d_denom": fa7_n,
...
def main() -> None:
    ...
    panel = panel.merge(pageviews[["month_id", "chatgpt_z"]], on="month_id", how="left")
    ...
    promoted = results_df.set_index("specification")
    memo = f"""# Unix Second-Setting Boundary Validation\n\nDate: April 4, 2026\n\n## Purpose\n\nThis second setting is not a full replication of the Stack Overflow contributor-role design. It is a boundary-setting validation on Unix Stack Exchange, chosen because it is a public technical troubleshooting platform with accepted answers and visible certification.\n\n## Data\n\n- Archive: [unix.stackexchange.com.7z](D:/AI alignment/projects/stackoverflow_chatgpt_governance/raw/unix.stackexchange.com.7z)\n- Extracted inputs: [Posts.xml](D:/AI alignment/projects/stackoverflow_chatgpt_governance/raw/unix_extract/Posts.xml), [Votes.xml](D:/AI alignment/projects/stackoverflow_chatgpt_governance/raw/unix_extract/Votes.xml)\n- Question-level file: [unix_second_setting_question_level.parquet](D:/AI alignment/projects/stackoverflow_chatgpt_governance/processed/unix_second_setting_question_level.parquet)\n- Tag-month panel: [unix_second_setting_tag_month_panel.csv](D:/AI alignment/projects/stackoverflow_chatgpt_governance/processed/unix_second_setting_tag_month_panel.csv)\n- Results: [unix_second_setting_results.csv](D:/AI alignment/projects/stackoverflow_chatgpt_governance/processed/unix_second_setting_results.csv)\n\n## Selected Tags\n\n{', '.join(SELECTED_TAGS)}\n\n## Read\n\nThe most important question is whether the second setting shows the same broad boundary pattern: a more AI-substitutable public queue becomes harder to convert from rapid response into later certified resolution.\n\nKey estimates:\n\n- `post_residualization`: `{promoted.loc['post_residualization', 'coef']:.4f}`, `p={promoted.loc['post_residualization', 'cluster_pval']:.4f}`\n- `post_gap`: `{promoted.loc['post_gap', 'coef']:.4f}`, `p={promoted.loc['post_gap', 'cluster_pval']:.4f}`\n- `post_accepted30`: `{promoted.loc['post_accepted30', 'coef']:.4f}`, `p={promoted.loc['post_accepted30', 'cluster_pval']:.4f}`\n- `external_gap`: `{promoted.loc['external_gap', 'coef']:.4f}`, `p={promoted.loc['external_gap', 'cluster_pval']:.4f}`\n- `external_accepted30`: `{promoted.loc['external_accepted30', 'coef']:.4f}`, `p={promoted.loc['external_accepted30', 'cluster_pval']:.4f}`\n- `external_first1d`: `{promoted.loc['external_first1d', 'coef']:.4f}`, `p={promoted.loc['external_first1d', 'cluster_pval']:.4f}`\n\n## Intended Use\n\nUse this second setting as a boundary-setting validation layer. If the gap and accepted-resolution terms move in the same direction as Stack Overflow while first-response terms remain weaker, that supports the manuscript's claim that public expert platforms can retain visible response while becoming more weakly coupled to certified resolution in the generative-AI period.\n"""
