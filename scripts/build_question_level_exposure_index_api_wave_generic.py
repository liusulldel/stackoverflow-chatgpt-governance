...
WAVE_NAME = os.environ.get("EXPOSURE_WAVE_NAME", "wave3").strip()
SEED = int(os.environ.get("EXPOSURE_WAVE_SEED", "628"))
...
MAX_BODY_CHARS = 1800
CONCURRENCY = int(os.environ.get("EXPOSURE_CONCURRENCY", "4"))
MAX_RETRIES = 8
RETRYABLE_STATUS = {429, 500, 502, 503, 504}

if not re.fullmatch(r"wave\d+", WAVE_NAME):
    raise ValueError(f"Unexpected wave name: {WAVE_NAME}")

SAMPLE_FILE = PROCESSED_DIR / f"question_level_exposure_{WAVE_NAME}_sample_576.csv"
BODY_FILE = PROCESSED_DIR / f"question_level_exposure_{WAVE_NAME}_sample_bodies.csv"
RAW_RESPONSE_FILE = PROCESSED_DIR / f"question_level_exposure_{WAVE_NAME}_api_responses.jsonl"
LABEL_FILE = PROCESSED_DIR / f"question_level_exposure_{WAVE_NAME}_labels.csv"
SUMMARY_JSON = PROCESSED_DIR / f"question_level_exposure_{WAVE_NAME}_summary.json"
SUMMARY_MD = PAPER_DIR / f"question_level_exposure_{WAVE_NAME}.md"
...
def historical_label_paths() -> list[Path]:
    return sorted(PROCESSED_DIR.glob("question_level_exposure*_labels.csv"))


def prior_question_ids() -> set[int]:
    ids: set[int] = set()
    for path in historical_label_paths():
        if path.name == LABEL_FILE.name:
            continue
        frame = pd.read_csv(path, usecols=["question_id"])
        ids.update(frame["question_id"].astype(int).tolist())
    return ids
...
def build_sample(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for (tag, post), group in df.groupby(["primary_tag", "post_chatgpt"], observed=False):
        ...
        if len(group) < TARGET_PER_STRATUM:
            raise RuntimeError(f"Not enough remaining questions in stratum {(tag, post)} for {WAVE_NAME}.")
...
def run_calls(sample: pd.DataFrame) -> pd.DataFrame:
    ...
    if failed_rows:
        raise RuntimeError(f"{WAVE_NAME} labeling still has {len(failed_rows)} failed rows after retries: {failed_rows[:3]}")
...
def append_ledger(count_increment: int, cumulative_total: int, notes: str) -> None:
    with CALL_LEDGER.open("a", encoding="utf-8") as handle:
        handle.write(
            f"exposure_index_labeling_{WAVE_NAME},local_llm_api,{count_increment},{cumulative_total},{RAW_RESPONSE_FILE.name},{notes}\n"
        )


def write_summary_md(summary: dict) -> None:
    lines = [
        f"# Question-Level Exposure Index {WAVE_NAME.title()}",
        "",
        "## API Labeling Summary",
        "",
        f"- Successful API calls in {WAVE_NAME}: `{summary['successful_api_calls']}`",
        ...
        "## Read",
        "",
        f"{WAVE_NAME.title()} expands the question-level exposure labeling pool with a non-overlapping stratified sample from the canonical 2025-backed design.",
    ]
...
def main() -> None:
    ...
    append_ledger(
        increment,
        cumulative,
        f"Completed {summary['main_labels']} main API labels and {summary['calibration_labels']} calibration API labels in {WAVE_NAME}.",
    )
