...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compute question-level complexity proxies for a Stack Exchange site dump.")
    parser.add_argument("--posts-xml", required=True)
    parser.add_argument("--questions-parquet", required=True)
...
def main() -> None:
    ...
    focal_questions = pd.read_parquet(args.questions_parquet, columns=["question_id"])
    ...
    for _, elem in ET.iterparse(posts_xml, events=("end",)):
        ...
        if matched % 100_000 == 0:
            print(f"Matched {matched:,}/{len(focal_ids):,} focal questions after {rows_seen:,} rows.", flush=True)
        elem.clear()

    features = pd.DataFrame(rows).sort_values("question_id").reset_index(drop=True)
    parquet_path = output_dir / f"{args.site_key}_question_complexity_features.parquet"
    csv_path = output_dir / f"{args.site_key}_question_complexity_features.csv"
    summary_path = output_dir / f"{args.site_key}_question_complexity_summary.json"
