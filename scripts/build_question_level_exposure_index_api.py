...
SAMPLE_FILE = PROCESSED_DIR / "question_level_exposure_sample_576.csv"
BODY_FILE = PROCESSED_DIR / "question_level_exposure_sample_bodies.csv"
RAW_RESPONSE_FILE = PROCESSED_DIR / "question_level_exposure_api_responses.jsonl"
LABEL_FILE = PROCESSED_DIR / "question_level_exposure_labels.csv"
SUMMARY_JSON = PROCESSED_DIR / "question_level_exposure_summary.json"
    ...
    for _event, elem in ET.iterparse(RAW_POSTS_XML, events=("end",)):
        ...
        if post_type == "1" and qid and int(qid) in needed:
            rows.append(
                {
                    "question_id": int(qid),
                    "body_excerpt": strip_html(elem.attrib.get("Body", "")),
                }
            )
...
def run_calls(sample: pd.DataFrame) -> pd.DataFrame:
    ...
    with ThreadPoolExecutor(max_workers=CONCURRENCY) as pool:
        futures = [pool.submit(worker, row._asdict() if hasattr(row, "_asdict") else row.to_dict()) for _, row in tasks.iterrows()]
...
def append_ledger(count_increment: int, cumulative_total: int, notes: str) -> None:
    with CALL_LEDGER.open("a", encoding="utf-8") as handle:
        handle.write(
            f"exposure_index_labeling,local_llm_api,{count_increment},{cumulative_total},question_level_exposure_api_responses.jsonl,{notes}\n"
        )
...
def write_summary_md(summary: dict) -> None:
    lines = [
        ...
        "## Read",
        "",
        "This file documents the first true question-level exposure/context-dependence labeling round under the canonical 2025-backed design. The labels are rubric-based and API-generated rather than human gold labels, so they should be read as a substantial measurement upgrade rather than final validation.",
    ]
...
def main() -> None:
    ...
    sample_ids = sample["question_id"].astype(int).unique().tolist()
    ...
    append_ledger(
        increment,
        cumulative,
        f"Completed {summary['main_labels']} main API labels and {summary['calibration_labels']} calibration API labels.",
    )
